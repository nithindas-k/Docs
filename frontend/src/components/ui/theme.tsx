  ...
  if (variant === "tabs") {
    return (
      <Tabs value={theme} onValueChange={setTheme} className={cn(className)}>
        <TabsList
          className={cn(
            "inline-flex items-center rounded-full p-1 bg-muted/50 backdrop-blur-sm shadow-inner"
          )}
        >
          {themes.map((themeOption) => {
            const Icon = themeIcons[themeOption]
            ...
            return (
              <TabsTrigger
                ...
                value={themeOption}
                className={cn(
                  "relative inline-flex items-center justify-center gap-1 transition-all duration-300",
                  size === "sm"
              ...
              >
                {isSelected && (
                  <motion.div
                    ...
                    className="absolute inset-0 rounded-full bg-background shadow-md border"
                    transition={{ type: "spring", stiffness: 400, damping: 25 }}
                  />
                )}
                <div className="relative z-10 flex items-center justify-center">
                  <Icon size={size === "sm" ? 14 : size === "md" ? 16 : 18} />
                  {showLabel && <span className="ml-2 text-xs font-semibold uppercase tracking-wider">{themeConfigs[themeOption].label}</span>}
                </div>
              </TabsTrigger>
            )
          })}
