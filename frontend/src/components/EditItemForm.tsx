...
export function EditItemForm({
  ...
  const [fields, setFields] = useState<Field[]>(itemFields.length > 0 ? itemFields : [{ key: '', value: '', isEncrypted: false }]);
  ...
  const handleManualScan = async () => {
    ...
    const scanFiles = uploadedFiles.filter(f => f.file && (f.file.size > 0 || f.preview));
    ...
    try {
      for (const fileItem of scanFiles) {
        ...
        let fileToScan: File | Blob;
        if (fileItem.file.size > 0) {
          fileToScan = fileItem.file;
        } else if (fileItem.preview) {
          try {
            const resp = await fetch(fileItem.preview);
            fileToScan = await resp.blob();
          } catch (e) {
            console.error('Failed to fetch existing photo for scan', e);
            continue;
          }
        } else continue;

        await handleScanSingleFile(fileToScan as File, fileItem.id);
      }
  ...
  const handleScanSingleFile = async (file: File, id: string) => {
    ...
    try {
      ...
      if (result.success && result.data) {
        ...
        const newFields: Field[] = data.fields || [];
  ...
  const handleCropDone = async (croppedImageUrl: string) => {
    ...
    try {
      ...
      setImageToCrop(null);
      setUploadedFiles(prev => [...prev.filter(f => f.preview && f.file.size === 0), {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div className="space-y-2">
        ...
        <Input type="text" placeholder={`Edit ${categoryName} title`} value={title} onChange={(e) => setTitle(e.target.value)} className="bg-accent/50" />
      </div>
